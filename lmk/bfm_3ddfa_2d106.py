...
class ParametricFaceModel:
    def __init__(self, 
        ...
        # add 3ddfa_106
        _3ddfa_106 = np.load("./3DMM/BFM/face_model.npy", allow_pickle=True).item()
        self.parallel = _3ddfa_106['parallel']
        self.v_parallel = - torch.ones(35709).type(torch.int64)
        for i in range(len(self.parallel)):
            self.v_parallel[self.parallel[i]]=i
        self.ldm106 = torch.tensor(model['ldm106'], requires_grad=False, dtype=torch.int64)
        ...
        # vertex indices for each mouth. starts from 0. [F,3]
        self.mouth_buf = model['mouth_triangles'].astype(np.int64) - 1
    ...
    def compute_for_mask(self, coeffs):
        """
        Return:
            face_vertex     -- torch.tensor, size (B, N, 3), in camera coordinate
            face_color      -- torch.tensor, size (B, N, 3), in RGB order
    ...
    def compute_landmark(self, coeffs):
        ...
        """
        # coef_dict   = self.split_coeff(coeffs)
        face_shape  = self.compute_shape(coeffs[:, :80], coeffs[:, 80: 144])
        rotation    = self.compute_rotation(coeffs[:, 224: 227])

        face_shape_transformed  = self.transform(face_shape, rotation, coeffs[:, 254:])
        face_vertex             = self.to_camera(face_shape_transformed)
        
        face_proj   = self.to_image(face_vertex)
        landmark    = face_proj[:, self.keypoints]
        return landmark

    def compute_landmark_simple(self, coeffs):
        """
        Return:
            landmark        -- torch.tensor, size (B, 68, 2), y direction is opposite to v direction
        Parameters:
            coeffs          -- torch.tensor, size (B, 150)
        """
        # coef_dict   = self.split_coeff(coeffs)
        face_shape  = self.compute_shape(coeffs[:, :80], coeffs[:, 80: 144])
        rotation    = self.compute_rotation(coeffs[:, 144: 147])

        face_shape_transformed  = self.transform(face_shape, rotation, coeffs[:, 147:])
        face_vertex             = self.to_camera(face_shape_transformed)
    ...
    # get the 468 landmark
    def compute_for_landmark_fit(self, id, exp, angles, trans, ret=None):
        """
        Return:
            face_vertex     -- torch.tensor, size (B, N, 3), in camera coordinate
            face_color      -- torch.tensor, size (B, N, 3), in RGB order
            landmark        -- torch.tensor, size (B, 468, 2), y direction is opposite to v direction
        Parameters:
            coeffs          -- torch.tensor, size (B, 257)
        """
